...
const TOKEN_KEY = 'bradpaws-admin-token';
...
function Login({ onLogin }: { onLogin: (s: Session) => void }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      onLogin({ token: body.token, slug: body.slug, displayName: body.displayName ?? body.slug });
    } catch {
...
function Snippets({ session }: { session: Session }) {
  const [snippets, setSnippets] = useState<{ script: string; iframe: string } | null>(null);
      ...
      <section>
        ...
        <label>
          ...
          <input
            ...
            onChange={(e) => setSettings({ ...settings, maxBoardingPets: Number(e.target.value) })}
          />
        </label>
      </section>
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/session', { headers: { Authorization: `Bearer ${token}` } })
      .then(async (res) => {
        if (!active) return;
        if (res.ok) {
          const body = (await res.json()) as { slug: string; displayName: string };
