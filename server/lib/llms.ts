...
export function buildLlmsTxt(
  ...
  lines.push(
    ...
    `- Availability & quote: GET ${origin}/api/${tenant.Slug}/availability?type=&option=&start=&end=&petIds= (email-code auth required; petIds is a comma-joined list of the caller's own pet ids, priced as the exact set requested). Money on the quote is named twice: estCostCents/extraTimeTotalCents/amountCents are INTEGER CENTS, and estCost/extraTimeTotal/extraTimeFees[].amount are the SAME money in whole dollars, retained for out-of-tree readers. Read the *Cents fields. An unpriced pet set carries NO cost field at all — do not substitute one.`,
    `- Booking requests: POST ${origin}/api/${tenant.Slug}/bookings (email-code auth; supports Idempotency-Key header). Every request starts as 'pending' and is confirmed or declined by the sitter — nothing is confirmed on creation. Every money field named *Cents is INTEGER CENTS: 7500 is $75.00, not $7,500.`,
    `- The caller's own bookings: GET ${origin}/api/${tenant.Slug}/bookings/mine (email-code auth; each row carries whether it can still be changed or cancelled, and the cancellation fee that would apply today). Every money field named *Cents is INTEGER CENTS: 7500 is $75.00, not $7,500.`,
    `- The caller's own household balance: GET ${origin}/api/${tenant.Slug}/account (email-code auth; what the household is expected to have paid, what it has paid, and the resulting balance — a negative balance means the household is in credit, not an error. Every figure on this payload is INTEGER CENTS, in a field named *Cents: balanceCents 7500 is $75.00, not $7,500.)`,
    `- Change a booking: PUT ${origin}/api/${tenant.Slug}/bookings/{id} (email-code auth; dates, pets, arrival time and intake answers only — the service and option cannot change, and a confirmed booking returns to 'pending' for the sitter to re-approve. No cancellation fee is assessed on a change. Every money field named *Cents is INTEGER CENTS: 7500 is $75.00, not $7,500.`,
    `- Cancel a booking: POST ${origin}/api/${tenant.Slug}/bookings/{id}/cancel (email-code auth; no request body — any cancellation fee is computed server-side from the sitter's stored policy and returned on the response, as cancellationFeeCents. Every money field named *Cents is INTEGER CENTS: 7500 is $75.00, not $7,500. A request the sitter has not confirmed is always free to withdraw.)`,
