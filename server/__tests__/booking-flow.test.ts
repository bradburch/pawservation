...
describe('booking flow', () => {
  ...
  it('completes identify → book → mine', async () => {
    ...
    const token = await identify(env, 'brad-paws', 'jess@example.com');

    const bookRes = await app.request(
      '/api/brad-paws/bookings',
      {
        method: 'POST',
  ...
  it('rejects a conflicting submit with 409 (server-side re-validation)', async () => {
    const { env } = createTestEnv();
    const token = await identify(env, 'brad-paws', 'jess@example.com');
    // Seed leaves only 1 boarding slot at Brad Paws over Jun 20-25; ask for 2 pets.
  ...
  it('sets embeddable headers on /embed/* and locked headers elsewhere', async () => {
    const { env } = createTestEnv();
    const embed = await app.request('/embed/brad-paws', {}, env);
    ...
    const api = await app.request('/api/brad-paws/config', {}, env);
