...
describe('species-count rates repo', () => {
  it('round-trips rates for one service option', async () => {
    ...
    await replaceServicePetRates(env.PAWSERVATION_DB, TENANT_A, 'walk', 'w30', [
      { mixKey: 'dog:2', rate: 35 },
      { mixKey: 'cat:1|dog:1', rate: 30 },
    ]);
    const rows = await listServicePetRates(env.PAWSERVATION_DB, TENANT_A);
  ...
  it('replace is scoped to one (serviceType, optionKey)', async () => {
    ...
    await replaceServicePetRates(env.PAWSERVATION_DB, TENANT_A, 'walk', 'w60', [
  ...
  it('is tenant-scoped', async () => {
    ...
    await replaceServicePetRates(env.PAWSERVATION_DB, TENANT_B, 'walk', 'w30', [
      { mixKey: 'dog:2', rate: 99 },
    ]);
    expect((await listServicePetRates(env.PAWSERVATION_DB, TENANT_A))[0].Rate).toBe(35);
    expect((await listServicePetRates(env.PAWSERVATION_DB, TENANT_B))[0].Rate).toBe(99);
  });

  it('keeps ServiceType separate from OptionKey — same mixKey, two service types', async () => {
    const { env } = createTestEnv();
    await replaceServicePetRates(env.PAWSERVATION_DB, TENANT_A, 'walk', 'standard', [
      { mixKey: 'dog:2', rate: 35 },
    ]);
...
describe('pet-group rates repo (upsert/delete-one — never whole-set replace)', () => {
  it('upsert creates a row, then updates it IN PLACE — same id, new rate', async () => {
    const { env } = createTestEnv();
    const first = await upsertPetGroupRate(env.PAWSERVATION_DB, TENANT_A, {
      serviceType: 'walk',
      optionKey: 'd30',
      groupKey: 'p_a,p_b',
      rate: 44,
    });
    const second = await upsertPetGroupRate(env.PAWSERVATION_DB, TENANT_A, {
      serviceType: 'walk',
      optionKey: 'd30',
      groupKey: 'p_a,p_b',
      rate: 50,
    ...
    const rows = await listPetGroupPricing(env.PAWSERVATION_DB, TENANT_A, 'walk');
  ...
  it('delete-one removes exactly one row and reports found/not-found', async () => {
    const { env } = createTestEnv();
    const { id } = await upsertPetGroupRate(env.PAWSERVATION_DB, TENANT_A, {
      serviceType: 'walk',
      optionKey: 'd30',
      groupKey: 'p_a',
      rate: 20,
    ...
    expect(await deletePetGroupRateById(env.PAWSERVATION_DB, TENANT_A, id)).toBe(true);
    expect(await deletePetGroupRateById(env.PAWSERVATION_DB, TENANT_A, id)).toBe(false);
    const rows = await listPetGroupPricing(env.PAWSERVATION_DB, TENANT_A, 'walk');
    expect(rows).toHaveLength(1);
  ...
  it('delete-one is tenant-scoped — tenant B cannot delete tenant A’s row', async () => {
    ...
    expect(await deletePetGroupRateById(env.PAWSERVATION_DB, TENANT_B, id)).toBe(false);
    expect(await listPetGroupPricing(env.PAWSERVATION_DB, TENANT_A, 'walk')).toHaveLength(1);
  });
  ...
  it('listAllPetGroupPricing spans services but never tenants', async () => {
    ...
    await upsertPetGroupRate(env.PAWSERVATION_DB, TENANT_B, {
      serviceType: 'walk',
      optionKey: 'd30',
      groupKey: 'p_a',
      rate: 99,
    });
    const all = await listAllPetGroupPricing(env.PAWSERVATION_DB, TENANT_A);
...
describe('deleteTenantCompletely clears both rate tables', () => {
  it('removes the deleted tenant’s rows and leaves the other tenant’s', async () => {
    ...
    for (const t of [TENANT_A, TENANT_B]) {
      await replaceServicePetRates(env.PAWSERVATION_DB, t, 'walk', 'w30', [
        { mixKey: 'dog:2', rate: 35 },
      ]);
      await upsertPetGroupRate(env.PAWSERVATION_DB, t, {
        serviceType: 'walk',
        optionKey: 'w30',
        groupKey: 'p_a',
        rate: 20,
    ...
    await deleteTenantCompletely(env.PAWSERVATION_DB, TENANT_A);
    expect(await listServicePetRates(env.PAWSERVATION_DB, TENANT_A)).toHaveLength(0);
    expect(await listPetGroupPricing(env.PAWSERVATION_DB, TENANT_A, 'walk')).toHaveLength(0);
    expect(await listServicePetRates(env.PAWSERVATION_DB, TENANT_B)).toHaveLength(1);
    expect(await listPetGroupPricing(env.PAWSERVATION_DB, TENANT_B, 'walk')).toHaveLength(1);
  });
});
