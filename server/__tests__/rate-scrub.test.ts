...
const seedRates = async (env: Env, tenantId: string) => {
  await replaceServicePetRates(env.PAWSERVATION_DB, tenantId, 'walk', 'd30', [
    { mixKey: 'dog:2', rate: 35 },
  ]);
  await replaceServicePetRates(env.PAWSERVATION_DB, tenantId, 'walk', 'd60', [
    { mixKey: 'dog:2', rate: 55 },
    { mixKey: 'cat:1|dog:1', rate: 50 },
  ]);
  await upsertPetGroupRate(env.PAWSERVATION_DB, tenantId, {
    serviceType: 'walk',
    optionKey: 'd30',
    groupKey: 'p_a,p_b',
    rate: 44,
...
describe('deleteService scrubs BOTH rate tables for that service', () => {
  it('removes the service’s rate rows, leaves a sibling service’s', async () => {
    ...
    await replaceServicePetRates(env.PAWSERVATION_DB, TENANT_A, 'checkin', 'd15', [
      { mixKey: 'dog:2', rate: 20 },
    ]);
    await deleteService(env.PAWSERVATION_DB, TENANT_A, 'walk');
    expect(await listPetGroupPricing(env.PAWSERVATION_DB, TENANT_A, 'walk')).toHaveLength(0);
    const mixes = await listServicePetRates(env.PAWSERVATION_DB, TENANT_A);
  ...
  it('is tenant-scoped', async () => {
    ...
    await deleteService(env.PAWSERVATION_DB, TENANT_A, 'walk');
    expect(await listServicePetRates(env.PAWSERVATION_DB, TENANT_B)).toHaveLength(3);
    expect(await listPetGroupPricing(env.PAWSERVATION_DB, TENANT_B, 'walk')).toHaveLength(2);
  });
});
...
describe('replaceServiceOptions scrubs rates for DROPPED option keys only', () => {
  it('a dropped option loses its rates in both tables; a kept option keeps them', async () => {
    ...
    await replaceServiceOptions(env.PAWSERVATION_DB, TENANT_A, 'walk', [opt('d30', 20)]);
    const mixes = await listServicePetRates(env.PAWSERVATION_DB, TENANT_A);
    expect(mixes.map((m) => m.OptionKey)).toEqual(['d30']);
    const groups = await listPetGroupPricing(env.PAWSERVATION_DB, TENANT_A, 'walk');
  ...
  it('an EMPTY option set clears every rate for the service', async () => {
    const { env } = createTestEnv();
    await seedRates(env, TENANT_A);
    await replaceServiceOptions(env.PAWSERVATION_DB, TENANT_A, 'walk', []);
  ...
  it('REGRESSION LOCK: a deleted option re-added under the same derived key does NOT resurrect its old rate', async () => {
    const { env } = createTestEnv();
    await seedRates(env, TENANT_A);
    await replaceServiceOptions(env.PAWSERVATION_DB, TENANT_A, 'walk', [opt('d30', 20)]); // d60 deleted
    await replaceServiceOptions(env.PAWSERVATION_DB, TENANT_A, 'walk', [
      opt('d30', 20),
...
describe('deletePetTypeAndScrub scrubs mix rates naming the dead species', () => {
  it('removes exactly the mixes containing the species, across options; group rows untouched', async () => {
    ...
    await deletePetTypeAndScrub(env.PAWSERVATION_DB, TENANT_A, 'rabbit');
    const mixes = await listServicePetRates(env.PAWSERVATION_DB, TENANT_A);
    ...
    expect(await listAllPetGroupPricing(env.PAWSERVATION_DB, TENANT_A)).toHaveLength(2);
  });
  ...
  it('a similarly-named species is NOT scrubbed (cat vs bobcat — no substring matching)', async () => {
    ...
    await createPetType(env.PAWSERVATION_DB, TENANT_A, 'bobcat', 'Bobcat');
    await replaceServicePetRates(env.PAWSERVATION_DB, TENANT_A, 'walk', 'd30', [
      { mixKey: 'bobcat:1', rate: 30 },
      { mixKey: 'cat:1', rate: 18 },
    ]);
    await deletePetTypeAndScrub(env.PAWSERVATION_DB, TENANT_A, 'bobcat');
    const mixes = await listServicePetRates(env.PAWSERVATION_DB, TENANT_A);
  ...
  it('is tenant-scoped', async () => {
    ...
    await replaceServicePetRates(env.PAWSERVATION_DB, TENANT_B, 'walk', 'd30', [
