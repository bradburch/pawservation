...
describe('tenant admin', () => {
  it('rejects missing, malformed, end-user, and wrong-tenant tokens', async () => {
    ...
    const missing = await app.request('/api/brad-paws/admin/settings', {}, env);
    expect(missing.status).toBe(401);

    const malformed = await app.request(
      '/api/brad-paws/admin/settings',
  ...
  it('settings edits are tenant-scoped and reflected live in the widget config (FR19)', async () => {
    ...
    const config = (await (await app.request('/api/brad-paws/config', {}, env)).json()) as {
      displayName: string;
      accentColor: string;
      maxBoardingPets: number;
      services: { type: string; options: { rate: number }[] }[];
    };
    expect(config.displayName).toBe('Brad Paws Deluxe');
  ...
  it('rejects a settings PUT with a bad service rate WITHOUT committing the rest (atomic validation)', async () => {
    ...
    expect(config.displayName).toBe('Brad Paws');
  ...
  it('capacity edits change availability outcomes (per-tenant max)', async () => {
    ...
    // The OTHER tenant's capacity is untouched by Brad Paws' change.
  ...
  it('blocked ranges block boarding and walks, and removal restores availability', async () => {
    ...
    await app.request(
      `/api/brad-paws/admin/blocked/${created.id}`,
  ...
  it('connecting a provider flips status to connected-stub, per tenant', async () => {
    const { env } = createTestEnv();
    await app.request(
      '/api/brad-paws/admin/providers/calendar/connect',
      { method: 'POST', headers: await auth(TENANT_A) },
      env,
    );
    const a = (await (
      await app.request('/api/brad-paws/admin/settings', { headers: await auth(TENANT_A) }, env)
    ...
    const unknown = await app.request(
      '/api/brad-paws/admin/providers/teleportation/connect',
      { method: 'POST', headers: await auth(TENANT_A) },
      env,
    );
