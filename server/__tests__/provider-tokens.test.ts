...
describe('provider token repo', () => {
  it('upserts tokens with connected status, then clears them', async () => {
    ...
    let conn = await getProviderConnection(env.PAWSERVATION_DB, TENANT_A, 'calendar');
    ...
    conn = await getProviderConnection(env.PAWSERVATION_DB, TENANT_A, 'calendar');
  ...
  it('setBookingGCalEventId writes the event id onto a booking', async () => {
    ...
    await setBookingGCalEventId(env.PAWSERVATION_DB, TENANT_A, 'b1', 'evt_xyz', null);
