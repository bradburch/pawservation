...
describe('BookingCharges repo', () => {
  it('inserts and lists a charge for a booking', async () => {
    ...
    const id = await insertBookingCharge(env.PAWBOOK_DB, TENANT_A, {
      bookingRequestId: bookingId,
      label: 'Vet visit',
      amount: 45,
    });
    expect(id).not.toBeNull();
    const rows = await listChargesForBooking(env.PAWBOOK_DB, TENANT_A, bookingId);
  ...
  it('refuses a booking belonging to another tenant', async () => {
    const { env } = createTestEnv();
    const bookingId = await makeBooking(env, TENANT_A);
    const id = await insertBookingCharge(env.PAWBOOK_DB, TENANT_B, {
      bookingRequestId: bookingId, // tenant A's booking
      label: 'Haircut',
      amount: 30,
    });
    expect(id).toBeNull();
    expect(await listChargesForBooking(env.PAWBOOK_DB, TENANT_A, bookingId)).toHaveLength(0);
  });
  ...
  it('deletes only the right charge, and only within its own booking and tenant', async () => {
    ...
    expect(await deleteBookingCharge(env.PAWBOOK_DB, TENANT_B, bookingId, id)).toBe(false);
    expect(await deleteBookingCharge(env.PAWBOOK_DB, TENANT_A, otherBookingId, id)).toBe(false);
    expect(await deleteBookingCharge(env.PAWBOOK_DB, TENANT_A, bookingId, id)).toBe(true);
    expect(await listChargesForBooking(env.PAWBOOK_DB, TENANT_A, bookingId)).toHaveLength(0);
  });
  ...
  it('lists every charge for a tenant and never another tenant’s', async () => {
    ...
    expect(await listChargesForTenant(env.PAWBOOK_DB, TENANT_A)).toHaveLength(1);
    expect(await listChargesForTenant(env.PAWBOOK_DB, TENANT_B)).toHaveLength(0);
  });
  ...
  it('is removed by deleteTenantCompletely', async () => {
    ...
    expect(await deleteTenantCompletely(env.PAWBOOK_DB, TENANT_A)).toBe(true);
    expect(await listChargesForTenant(env.PAWBOOK_DB, TENANT_A)).toHaveLength(0);
  });
});
