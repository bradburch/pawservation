...
describe('payments repo', () => {
  ...
  it('deletePayment is tenant-scoped and requires the matching booking id', async () => {
    ...
    expect(await deletePayment(env.PAWSERVATION_DB, TENANT_B, bookingId, paymentId)).toBe(false);
    // Right tenant, WRONG booking id in the pair: refused (404 at the route, never a silent delete).
    expect(await deletePayment(env.PAWSERVATION_DB, TENANT_A, otherBookingId, paymentId)).toBe(
      false,
    );
    // Correct pair: deleted.
    expect(await deletePayment(env.PAWSERVATION_DB, TENANT_A, bookingId, paymentId)).toBe(true);
    expect(await listPaymentsForBooking(env.PAWSERVATION_DB, TENANT_A, bookingId)).toHaveLength(0);
    // Gone now: a second delete reports false.
    expect(await deletePayment(env.PAWSERVATION_DB, TENANT_A, bookingId, paymentId)).toBe(false);
  });
