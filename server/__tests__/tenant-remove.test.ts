...
function seedFullTenant(raw: DatabaseSync, t: string, slug: string) {
  ...
  raw.exec(
    `INSERT INTO AllowedSitters (Email, ClaimedAt, TenantId) VALUES ('${t}@x.test','2026-01-01','${t}');`,
  );
  raw.exec(
    `INSERT INTO TenantUsers (Id, TenantId, Email, PasswordHash) VALUES ('${t}_u','${t}','${t}@x.test','h');`,
  );
  raw.exec(
  ...
  raw.exec(
    `INSERT INTO LoginCodes (Id, TenantId, EndUserId, Code, ExpiresAt) VALUES ('${t}_lc','${t}','${t}_eu','123456','2026-12-31');`,
  );
  raw.exec(
    `INSERT INTO EndUserPets (Id, TenantId, EndUserId, Name, PetType) VALUES ('${t}_pet','${t}','${t}_eu','Rex','dog');`,
  );
  raw.exec(
    `INSERT INTO PetOwners (TenantId, PetId, EndUserId) VALUES ('${t}','${t}_pet','${t}_eu');`,
  );
  raw.exec(
    `INSERT INTO BookingRequests (Id, TenantId, EndUserId, ServiceType, StartDate, Status) VALUES ('${t}_b','${t}','${t}_eu','boarding','2026-07-20','confirmed');`,
  );
  raw.exec(
  ...
  // A tenant access token (0016) FKs to BOTH Tenants and TenantUsers, so it must go before either.
  raw.exec(
    `INSERT INTO TenantAccessTokens (Id, TenantId, TenantUserId, Name, TokenHash) VALUES ('${t}_tat','${t}','${t}_u','CLI','${t}_tathash');`,
  );
}
...
describe('deleteTenantCompletely', () => {
  it('deletes every tenant-keyed row + allowlist + tenant, leaves other tenants untouched', async () => {
    ...
    expect(
      (
        raw
          .prepare("SELECT COUNT(*) AS n FROM BookingRequestPets WHERE BookingRequestId='t_gone_b'")
          .get() as { n: number }
      ).n,
    ).toBe(0);
    expect(
    ...
    expect(
      (raw.prepare("SELECT COUNT(*) AS n FROM Tenants WHERE Id='t_gone'").get() as { n: number }).n,
    ).toBe(0);

    // Isolation: the other tenant is fully intact.
    for (const table of TENANT_TABLES) expect(countFor(raw, table, 't_keep')).toBe(1);
    expect(
      (raw.prepare("SELECT COUNT(*) AS n FROM Tenants WHERE Id='t_keep'").get() as { n: number }).n,
    ).toBe(1);
    expect(
      (
        raw
          .prepare("SELECT COUNT(*) AS n FROM BookingRequestPets WHERE BookingRequestId='t_keep_b'")
          .get() as { n: number }
      ).n,
    ).toBe(1);
    expect(
      (
        raw.prepare("SELECT COUNT(*) AS n FROM AllowedSitters WHERE TenantId='t_keep'").get() as {
          n: number;
        }
      ).n,
    ).toBe(1);
